...
async def compile_document(
    ...
    if req.project_id and req.doc_id:
        ...
        docs_result = await db.execute(select(Document).where(Document.project_id == req.project_id))
        project_documents = docs_result.scalars().all()

    if active_doc and project_documents:
        ...
        result = await compile_latex(req.content, req.output_format)
